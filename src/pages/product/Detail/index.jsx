...
function ProductDetail() {
  ...
  const totalThumbnailPages = Math.ceil((product?.images.length + 1) / thumbnailsPerPage);
  ...
  const handlePrevSlide = () => setCurrentSlide((prev) => (prev === 0 ? media.length - 1 : prev - 1));
  const handleNextSlide = () => setCurrentSlide((prev) => (prev === media.length - 1 ? 0 : prev + 1));
  const handleThumbnailClick = (index) => setCurrentSlide(index);

  const handlePrevThumbnailPage = () => setThumbnailPage((prev) => Math.max(prev - 1, 0));
  const handleNextThumbnailPage = () => setThumbnailPage((prev) => Math.min(prev + 1, totalThumbnailPages - 1));

  const handlePrevRelatedPage = () => setRelatedPage((prev) => Math.max(prev - 1, 0));
  const handleNextRelatedPage = () => setRelatedPage((prev) => Math.min(prev + 1, relatedTotalPages - 1));

  return (
    <div className="container mx-auto p-4">
      <div className="text-center mb-10">
        <h1 className="text-4xl font-bold">Thông tin chi tiết về {product.name}</h1>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-10 mb-14 items-start">
        ...
        <div className="w-full md:w-1/3 relative flex flex-col items-center">
          <div className="relative w-full h-96">
            <button onClick={handlePrevSlide} className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-gray-300 p-2 rounded-full">
              {"<"}
            </button>
            <button onClick={handleNextSlide} className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-gray-300 p-2 rounded-full">
              {">"}
            </button>
          ...
          <div className="flex items-center mt-4 space-x-2 mb-9">
            {thumbnailPage > 0 && (
              <button onClick={handlePrevThumbnailPage} className="p-2 bg-gray-300 rounded-lg">
                {"<"}
              </button>
            )}
            {media
              ...
              .map((item, index) => (
                <img
                  ...
                  src={item.url}
                  alt="Thumbnail"
                  onClick={() => handleThumbnailClick(thumbnailPage * thumbnailsPerPage + index)}
            ...
            {thumbnailPage < totalThumbnailPages - 1 && (
              <button onClick={handleNextThumbnailPage} className="p-2 bg-gray-300 rounded-lg">
                {">"}
              </button>
            )}
          </div>
        ...
        <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
          ...
          <tbody className="text-gray-700 text-sm">
            {product.productAttributes.map((attr) => (
              <tr key={attr.id} className="border-b border-gray-200 hover:bg-gray-100">
