...
const PurchaseRequestService = {
  ...
  postPurchaseRequest(productId, message) {
    return service(
      axios.post(getApiUrl(`/purchase-requests/${productId}`), message, {
        headers: {
  ...
  getPurchaseRequestsByProductId(productId) {
    return service(
      axios.get(getApiUrl(`/purchase-requests/product/${productId}`))
    );
  },
  ...
  acceptPurchaseRequest(purchaseRequestId) {
    return service(
      axios.put(getApiUrl(`/purchase-requests/${purchaseRequestId}/accept`))
    );
  },
  ...
  rejectPurchaseRequest(purchaseRequestId) {
    return service(
      axios.put(getApiUrl(`/purchase-requests/${purchaseRequestId}/reject`))
    );
  },
  ...
  deletePurchaseRequest(purchaseRequestId) {
    return service(
      axios.delete(getApiUrl(`/purchase-requests/${purchaseRequestId}`))
    );
  },
};
