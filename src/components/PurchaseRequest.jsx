...
const PurchaseRequest = ({ isBuyer }) => {
  ...
  const handleAction = async (purchaseRequestId, action) => {
    ...
    try {
      let response;
      if (action === "accept") {
        response = await purchaseRequestService.acceptPurchaseRequest(
          purchaseRequestId
        );
        message.success("Yêu cầu mua đã được chấp nhận.");
      } else if (action === "reject") {
        response = await purchaseRequestService.rejectPurchaseRequest(
          purchaseRequestId
        );
        message.success("Yêu cầu mua đã bị từ chối.");
      } else if (action === "delete") {
        response = await purchaseRequestService.deletePurchaseRequest(
          purchaseRequestId
        );
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div style={{ flex: "2", paddingRight: "20px" }}>
                  <p>
                    ...
                    {request.productName || "Không có"}
                  </p>
                  <p>
                    <strong>Giá:</strong>{" "}
                    {request.productPrice
                      ? `${request.productPrice} VND`
                      : "Không có"}
                  </p>
                  <p>
                    <strong>Mô tả:</strong>
                  ...
                  <p>
                    ...
                    {request.buyerEmail || "Không có email"}
                  </p>
                  <p>
                    <strong>Tên người mua:</strong>{" "}
                    {request.buyerName || "Không có tên"}
                  </p>
                  <p>
                    <strong>Tin nhắn:</strong> {request.message}
                  </p>
                  <p>
                    <strong>Ngày đăng:</strong> {formatDate(request.postedAt)}
                  </p>
                  <p>
  ...
  const getTabs = (requests) => {
    ...
    return [
      {
        label: `Chờ xử lý (${groupedRequests.pending.length})`,
        key: "1",
        children: renderList(groupedRequests.pending),
      },
      {
        label: `Đã chấp nhận (${groupedRequests.accepted.length})`,
        key: "2",
        children: renderList(groupedRequests.accepted),
      },
      {
        label: `Đã từ chối (${groupedRequests.rejected.length})`,
        key: "3",
        children: renderList(groupedRequests.rejected),
      },
    ];
  };
  ...
  return (
    <div>
      <h2>
        {isBuyer ? "Yêu cầu mua của người mua" : "Yêu cầu mua của người bán"}
      </h2>
